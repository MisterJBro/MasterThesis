...
/// Core algorithm of AZ, with all its functionality. Shareable between threads.
#[derive(Debug)]
pub struct AZCore {
...
impl AZCore {
    pub fn new(state: State, capacity: usize) -> AZCore {
        let arena = Arena::new(capacity);

        // Add root node
        let legal_acts = AZCore::get_legal_actions(&state);
    ...
    pub fn expand<'a>(&'a self, node: &'a Node) -> Result<&'a Node, &'static str> {
        ...
        let next_legal_acts = AZCore::get_legal_actions(&next_state);
    ...
    /// Get legal actions of state, the order is important! Children are added based on order. Random shuffle for AZ
    #[inline]
