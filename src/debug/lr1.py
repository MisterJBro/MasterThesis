...
if __name__ == '__main__':
    ...
    lrs = [5e-5, 1e-5, 5e-4, 1e-4, 5e-3, 1e-3, 5e-2, 1e-2, 5e-1, 1e-1, 2e-1, 1]
    pgs_lr = lrs[int(config["job_id"])]
    ...
    for i1, name1 in enumerate(names):
        for j1, name2 in enumerate(names):
            ...
            if i1 == 1 or j1 == 1:
                pgs_ext_obj = PGS(config, policy, pgs_lr=pgs_lr, dyn_length=False, scale_vals=True, expl_entr=True, expl_kl=True, visit_counts=True, update=True)
